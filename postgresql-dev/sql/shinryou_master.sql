create table shinryou_master (
	...
	shuukeisaki char(3) not null check(shuukeisaki in (
	...
	houkatsukensa char(2) not null check(houkatsukensa in (
		'00', '01', '02', '03', '05', '06', '07', '08', '09', '10', '11')),
...
create or replace function check_shinryou_master_fun() returns trigger as $$
	declare
		count integer;
	begin
		select * into count from shinryou_master where shinryoucode = new.shinryoucode 
		...
		if count > 0 then
			raise exception 'conflicing shinryou_master row exists';
		end if;
		return new;
	end;
$$ language plpgsql;
