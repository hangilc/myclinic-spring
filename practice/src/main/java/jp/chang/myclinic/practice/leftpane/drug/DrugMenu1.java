...
class DrugMenu1 extends JPanel {
    ...
    DrugMenu1(VisitDTO visit, int currentVisitId, int tempVisitId){
    ...
    private void doNewDrug(VisitDTO visit){
        ...
        if( workPane != null ){
            if( workPane instanceof DrugNew1){
                remove(workPane);
                workPane = null;
                repaint();
                revalidate();
        ...
        DrugNew1 drugNew = new DrugNew1(visit);
        drugNew.setCallback(new DrugNew1.Callback(){
            @Override
