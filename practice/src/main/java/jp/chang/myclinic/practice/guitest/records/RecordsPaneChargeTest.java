...
public class RecordsPaneChargeTest extends GuiTestBase {

    public RecordsPaneChargeTest(Stage stage, StackPane main) {
        super(stage, main);
    }
    ...
    public void disp(){
        ...
        VisitDTO visit1 = frontend.startVisit(patient.patientId, prevDays(0)).join();
        Context.currentPatientService = new CurrentPatientService();
        frontend.startExam(visit1.visitId);
        frontend.enterShinryouByName(visit1.visitId, "初診").join();
        frontend.endExam(visit1.visitId, 0).join();
