...
probe_encoding() {
	head -18 "$1" | grep -oP '^/\*!\d+ SET NAMES \K(\w+)'
}
...
docker_process_init_files() {
	...
	for f; do
		case "$f" in
			...
			*.sql)    mysql_note "$0: running $f"
						local sqlenc=$(probe_encoding "$f")
						if [ -n "$sqlenc" ]; then
							_INTERNAL_INIT_SQL_CHARSET="$sqlenc"
						fi
						docker_process_init_sql < "$f"
						echo
						;;
			*.sql.gz) mysql_note "$0: running $f"
						gunzip -c "$f" | docker_process_init_sql
						echo
						;;
